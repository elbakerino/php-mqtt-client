 ...
 */
class Mqtt311MessageProcessor extends BaseMessageProcessor implements MessageProcessor
{
    ...
    public function buildConnectMessage(ConnectionSettings $connectionSettings, bool $useCleanSession = false): string
    {
        ...
        //$buffer  = $this->buildLengthPrefixedString('mqtts');
        $buffer  = $this->buildLengthPrefixedString('MQTT');
        $buffer .= chr(0x04); // protocol version (3)
    ...
    public function handleConnectAcknowledgement(string $message): void
    {
        ...
        switch ($errorCode) {
            ...
            case 0x01:
                $this->logger->error('The broker does not support MQTT v3.1.1', $logContext);
                throw new ConnectingToBrokerFailedException(
                    ConnectingToBrokerFailedException::EXCEPTION_CONNECTION_PROTOCOL_VERSION,
                    'The configured broker does not support MQTT v3.1.1'
                );
    ...
    ): string
    {
        ...
        #$header .= $this->buildLengthPrefixedString('name=lead_create'); 
    ...
    protected function parseAndValidateSubscribeAcknowledgementMessage(string $data): Message
    {
        ...
        foreach ($acknowledgements as $acknowledgement) {
            if (!in_array($acknowledgement, [0, 1, 2])) {
                throw new InvalidMessageException('Received subscribe acknowledgement `'.$acknowledgement.'` with invalid QoS values from the broker.');
            }
        }
